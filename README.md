...
如果 `AgentCard.authentication.credentials` 存在，默认会被发现端点自动脱敏。要返回完整字段，请设置环境变量并带上认证头：

```bash
export A2A_AGENT_CARD_TOKEN=local-dev-token
...
- `A2A_DISCOVERY_CACHE_SECONDS`（默认 60）
- `A2A_DISCOVERY_RPS`（默认 5，每秒）
